...
p_load(tidyverse, lme4, marginaleffects, glmmTMB, mice, ordinal, modelsummary, lubridate, data.table, mctq, glue)
...
selfreport <- selfreport_versions$imputed
...
build_gaming_inputs <- function(selfreport_data) {
  gamingBiweekly <- calculate_gaming_windows(selfreport_data |> dplyr::select(-msf_sc_centered), gaming_sessions) |>
    left_join(selfreport_data |> dplyr::select(pid, msf_sc_centered) |> filter(!is.na(msf_sc_centered)) |> distinct(pid, .keep_all = TRUE),
              by = "pid") |>
