...
# For total sample, use selfreport_saved (imputed, all valid pids) where available
# Participants in total_sample but not in selfreport_saved will have NA outcomes
outcomes_total <- compute_outcomes(total_sample, selfreport_saved)
...
chrono_total <- compute_chronotype(total_sample, selfreport_saved)
...
summary_rows <- bind_rows(summary_rows,
  tibble(Characteristic = "    MCTQ-MSF~sc~ (HH:MM)†",
...
diary_model_h1 <- clm(diary_h1_formula, data = diary_ocat, link = "logit", Hess = TRUE)
attr(diary_model_h1, "n_participants_pid") <- length(unique(diary_ocat$pid))
saveRDS(diary_model_h1, "output/models/diary_model_h1_sleepquality.rds")

# Spline comparison (df = 2:6)
...
diary_linear_aic <- AIC(diary_model_h1)
...
diary_model_h2 <- clm(diary_h2_formula, data = diary_ocat_h2, link = "logit", Hess = TRUE)
attr(diary_model_h2, "n_participants_pid") <- length(unique(diary_ocat_h2$pid))
...
if (!is.null(diary_model_h1_loaded) && !is.null(diary_model_h2_loaded)) {
  ...
  modelsummary(
    ...
    notes = c(
      ...
      "LN = late-night. Confidence intervals shown in brackets.",
      "Cumulative link models (logit link) on 5-level ordinal sleep quality.",
