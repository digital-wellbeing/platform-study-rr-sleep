...
diary_h1_formula <- sleep_quality_ord ~ late_night_within + late_night_between +
  age_scaled + bmi_scaled + SES_index_scaled + gender + is_weekend + (1 | pid)

diary_model_h1 <- clmm(diary_h1_formula, data = diary_ocat, link = "logit", Hess = TRUE)
...
diary_covariate_terms <- c("late_night_between", "age_scaled", "bmi_scaled",
                           "SES_index_scaled", "gender", "is_weekend")

diary_spline_fits <- purrr::map(2:6, \(df) {
  f <- as.formula(paste(
    "sleep_quality_ord ~",
    paste(c(sprintf("splines::ns(late_night_within, df = %d)", df),
...
diary_clm_linear <- clm(
  sleep_quality_ord ~ late_night_within + late_night_between +
    age_scaled + bmi_scaled + SES_index_scaled + gender + is_weekend,
  data = diary_ocat, link = "logit", Hess = TRUE
)
...
diary_h2_formula <- sleep_quality_ord ~
  late_night_within * chronotype_centered +
  late_night_between * chronotype_centered +
  age_scaled + bmi_scaled + SES_index_scaled + gender + is_weekend + (1 | pid)

diary_model_h2 <- clmm(diary_h2_formula, data = diary_ocat_h2, link = "logit", Hess = TRUE)
...
# H1 effects (ORs)
diary_h1_within <- get_h2_interaction(diary_h1_path, "late_night_within",
                                       exponentiate = TRUE, label_if_exp = "OR")
diary_h1_between <- get_h2_interaction(diary_h1_path, "late_night_between",
                                        exponentiate = TRUE, label_if_exp = "OR")

# H2 effects (ORs)
diary_h2_within_chrono <- get_h2_interaction(diary_h2_path,
  "late_night_within:chronotype_centered", exponentiate = TRUE, label_if_exp = "OR")
diary_h2_between_chrono <- get_h2_interaction(diary_h2_path,
  "late_night_between:chronotype_centered", exponentiate = TRUE, label_if_exp = "OR")
diary_h2_chrono_main <- get_h2_interaction(diary_h2_path,
  "chronotype_centered", exponentiate = TRUE, label_if_exp = "OR")
...
For the chronotype moderation model (H2), the interaction between within-person late-night gaming and chronotype was `r diary_h2_within_chrono`, and the interaction between between-person late-night gaming and chronotype was `r diary_h2_between_chrono`. The main effect of chronotype was `r diary_h2_chrono_main`. Consistent with the panel-level H2 findings, there was little evidence that chronotype substantially moderated the association between late-night gaming and daily sleep quality at either the within- or between-person level.
...
if (!is.null(diary_model_h1_loaded) && !is.null(diary_model_h2_loaded)) {
  ...
  diary_coef_map <- c(
    "late_night_within" = "LN gaming within-person (min)",
    "late_night_between" = "LN gaming between-person (min)",
    "chronotype_centered" = "Chronotype (centered)",
    "late_night_within:chronotype_centered" = "LN within x Chronotype",
    "late_night_between:chronotype_centered" = "LN between x Chronotype",
    "age_scaled" = "Age (scaled)",
    "bmi_scaled" = "BMI (scaled)",
    "SES_index_scaled" = "SES (scaled)"
  )

  # Detect gender and region terms from both models
  all_coef_names <- unique(c(
    names(coef(diary_model_h1_loaded)),
    names(coef(diary_model_h2_loaded))
  ))
  threshold_pattern <- "^[0-9]|^Very poor|^Poor|^Fair|^Good"
  non_threshold <- all_coef_names[!grepl(threshold_pattern, all_coef_names)]

  gender_terms <- non_threshold[grepl("^gender", non_threshold)]
  gender_map <- setNames(
    paste0("Gender: ", gsub("^gender", "", gender_terms)),
    gender_terms
  )
  ...
  diary_coef_map <- c(diary_coef_map, gender_map, region_map, weekend_map)
  ...
  modelsummary(
    ...
    notes = c(
      ...
      "LN = late-night. Confidence intervals shown in brackets.",
      "Cumulative link mixed models (logit link, random intercept for participant) on 5-level ordinal sleep quality.",
