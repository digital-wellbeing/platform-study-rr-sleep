...
#| fig-cap: "Predicted probability of poor sleep quality as a function of average late-night gaming (diary study, between-person effect). The line shows the probability of reporting poor sleep quality (Poor or Very poor on the 5-level ordinal scale) across the range of participants' average daily late-night gaming, centered at the grand mean. The ribbon represents 95% confidence intervals. Vertical reference lines mark the median (dotted), mean (dashed), 75th percentile (dotted), and 90th percentile (dashed) of the sample distribution. The marginal density plot (top panel) shows the distribution of between-person late-night gaming. Predictions are derived from the diary H1 CLMM (random intercept for participant, probit link) adjusting for age, BMI, SES, and weekday vs. weekend, with the within-person component held at zero (i.e., at the participant's own mean)."
#| fig-width: 9
#| fig-height: 6
...
if (file.exists("output/models/diary_model_h1_sleepquality.rds")) {
  ...
  # Between-person exceedance (within held at person mean = 0)
  emm_between <- emmeans(
    diary_h1_fig, ~ ln_between_10 | cut,
    at = list(ln_between_10 = ln_seq, ln_within_10 = 0),
    mode = "exc.prob"
  )
  df_between <- as.data.frame(emm_between) |>
    mutate(gaming_min = ln_between_10 * 10) |>
    filter(as.character(cut) == "3|4")  # P(Poor or Very poor) = P(Y > "Fair")
