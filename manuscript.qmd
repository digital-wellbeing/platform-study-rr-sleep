...
emm_spec <- emmeans(
  h1a_model,
  ~ ln_monthly_avg_minutes_played_10 | cut,
  at = list(ln_monthly_avg_minutes_played_10 = ln_sequence_10),
  mode = "exc.prob"
)

# Convert to dataframe and scale back to original minutes for display
emm_df <- as.data.frame(emm_spec) |>
  mutate(ln_monthly_avg_minutes_played = ln_monthly_avg_minutes_played_10 * 10)
...
#| fig-cap: "Predicted probability of poor sleep quality as a function of late-night gaming (diary study). The left column shows the within-person effect (daily deviation from a participant's own mean late-night gaming); the right column shows the between-person effect (participant mean centered at the grand mean). Top panels show the marginal distribution of each gaming component; bottom panels show the predicted probability of reporting poor sleep (Poor or Very poor on the 5-level ordinal scale). Ribbons represent 95% confidence intervals. Vertical reference lines mark the median (dotted), mean (dashed), 75th percentile (dotted), and 90th percentile (dashed) of the sample distribution. Predictions are derived from the diary H1 CLMM (random intercept for participant, probit link) adjusting for age, BMI, SES, and weekday vs. weekend, with the complementary gaming component held at its grand mean (zero)."
#| fig-width: 11
#| fig-height: 6
#| code-summary: "Diary: probability of poor sleep quality by within- and between-person late-night gaming"
#| echo: false
#| message: false
#| warning: false
...
if (file.exists("output/models/diary_model_h1_sleepquality.rds")) {
  ...
  # Prediction grids (per-10-min units: -6 to 9 = -60 to 90 min)
  ln_seq <- seq(-6, 9, by = 0.1)

  # Within-person exceedance (between held at grand mean = 0)
  emm_within <- emmeans(
    diary_h1_fig, ~ ln_within_10 | cut,
    at = list(ln_within_10 = ln_seq, ln_between_10 = 0),
    mode = "exc.prob"
  )
  df_within <- as.data.frame(emm_within) |>
    mutate(gaming_min = ln_within_10 * 10) |>
  ...
  # Distribution summary stats (in minutes)
  get_dist_stats <- function(vals) {
    list(
  ...
  within_stats  <- get_dist_stats(diary_ocat$late_night_within)
  between_stats <- get_dist_stats(diary_ocat$late_night_between)

  # Helper: density distribution panel
  make_density_panel <- function(vals, stats, x_range, x_breaks, title_label) {
    ggplot(data.frame(x = vals[!is.na(vals)]), aes(x = x)) +
      geom_density(fill = "gray70", color = "gray50", alpha = 0.6) +
      geom_vline(xintercept = stats$median, linetype = "dotted", color = "gray40", linewidth = 0.5) +
      geom_vline(xintercept = stats$mean,   linetype = "dashed",  color = "gray40", linewidth = 0.5) +
      geom_vline(xintercept = stats$p75,    linetype = "dotted", color = "gray40", linewidth = 0.5) +
      geom_vline(xintercept = stats$p90,    linetype = "dashed",  color = "gray40", linewidth = 0.5) +
      scale_x_continuous(breaks = x_breaks, labels = paste0(x_breaks, " min"), limits = x_range) +
      labs(title = title_label, y = "Density", x = NULL) +
      theme_minimal(base_size = 11) +
      theme(
        panel.grid.minor  = element_blank(),
        axis.text.x       = element_blank(),
  ...
  # Helper: exceedance probability panel
  make_exceedance_panel <- function(df, stats, x_range, x_breaks, x_label) {
    y_top <- max(df$asymp.UCL, na.rm = TRUE)
    y_ann <- y_top * 0.88

    ggplot(df, aes(x = gaming_min, y = exc.prob)) +
      geom_vline(xintercept = stats$median, linetype = "dotted", color = "gray40", linewidth = 0.5) +
      geom_vline(xintercept = stats$mean,   linetype = "dashed",  color = "gray40", linewidth = 0.5) +
      geom_vline(xintercept = stats$p75,    linetype = "dotted", color = "gray40", linewidth = 0.5) +
      geom_vline(xintercept = stats$p90,    linetype = "dashed",  color = "gray40", linewidth = 0.5) +
      annotate("text", x = stats$median, y = y_ann, label = "Median",
               angle = 90, vjust = -0.5, size = 3, color = "gray40") +
      annotate("text", x = stats$mean,   y = y_ann, label = "Mean",
               angle = 90, vjust = -0.5, size = 3, color = "gray40") +
      annotate("text", x = stats$p75,    y = y_ann, label = "75th %ile",
               angle = 90, vjust = -0.5, size = 3, color = "gray40") +
      annotate("text", x = stats$p90,    y = y_ann, label = "90th %ile",
               angle = 90, vjust = -0.5, size = 3, color = "gray40") +
      geom_ribbon(aes(ymin = asymp.LCL, ymax = asymp.UCL),
      ...
      labs(x = x_label, y = "P(Poor or Very poor sleep)") +
      theme_minimal(base_size = 11) +
      theme(panel.grid.minor = element_blank(), plot.margin = margin(0, 5, 5, 5))
  }

  p_within_dist  <- make_density_panel(
    diary_ocat$late_night_within,  within_stats,  x_range, x_breaks, "Within-Person"
  )
  p_between_dist <- make_density_panel(
    diary_ocat$late_night_between, between_stats, x_range, x_breaks, "Between-Person"
  )
  p_within_main  <- make_exceedance_panel(
    df_within,  within_stats,  x_range, x_breaks,
    "Within-Person LN Gaming Deviation (min/day)"
  )
  p_between_main <- make_exceedance_panel(
    df_between, between_stats, x_range, x_breaks,
    "Between-Person LN Gaming Deviation (min/day)"
  )

  (p_within_dist | p_between_dist) /
    (p_within_main | p_between_main) +
    plot_layout(heights = c(1, 3)) +
    plot_annotation(
