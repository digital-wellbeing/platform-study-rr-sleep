...
modelsummary(
    ...
    add_rows = get_icc_rows(h1_models[[main_version]]),
...
if (length(supp_versions) > 0) {
  ...
  for (ver in supp_versions) {
    ...
    modelsummary(
      ...
      gof_omit = "AIC|BIC|Log|RMSE|R2|ICC",
      coef_map = coef_map_h1,
      add_rows = get_icc_rows(h1_models[[ver]]),
      notes = c("+ p < 0.10, * p < 0.05, ** p < 0.01, *** p < 0.001",
                "LN = late-night. Confidence intervals shown in brackets.",
                "ICC = Intraclass Correlation Coefficient (adjusted)."),
...
p_main <- ggplot(plot_data, aes(x = ln_monthly_avg_minutes_played, y = exc.prob)) +
  ...
  labs(
    x = "Average Daily Late-Night Gaming",
    y = "Probability of Poor Sleep Quality"
  ) +
  theme_minimal(base_size = 11) +
  theme(
    panel.grid.minor = element_blank(),
...
modelsummary(
  ...
  add_rows = get_icc_rows(h2_models[[main_version]]),
  notes = c("+ p < 0.10, * p < 0.05, ** p < 0.01, *** p < 0.001",
            "LN = late-night. Confidence intervals shown in brackets.",
            "ICC = Intraclass Correlation Coefficient (adjusted)."),
    ...
    for (ver in supp_versions) {
      if (!is.null(h2_models[[ver]])) {
        ...
        modelsummary(
          ...
          gof_omit = "AIC|BIC|Log|RMSE|R2|ICC",
          coef_map = coef_map_h2,
          add_rows = get_icc_rows(h2_models[[ver]]),
          notes = c("+ p < 0.10, * p < 0.05, ** p < 0.01, *** p < 0.001",
                    "LN = late-night. Confidence intervals shown in brackets.",
                    "ICC = Intraclass Correlation Coefficient (adjusted)."),
...
if ("completecase" %in% names(h1_models)) {
  modelsummary(
    ...
    gof_omit = "AIC|BIC|Log|RMSE|R2|ICC",
    coef_map = coef_map_h1,
    add_rows = get_icc_rows(h1_models[["completecase"]]),
    notes = c("+ p < 0.10, * p < 0.05, ** p < 0.01, *** p < 0.001",
              "LN = late-night. Confidence intervals shown in brackets.",
              "ICC = Intraclass Correlation Coefficient (adjusted)."),
...
if ("completecase" %in% names(h2_models)) {
  modelsummary(
    ...
    gof_omit = "AIC|BIC|Log|RMSE|R2|ICC",
    coef_map = coef_map_h2,
    add_rows = get_icc_rows(h2_models[["completecase"]]),
    notes = c("+ p < 0.10, * p < 0.05, ** p < 0.01, *** p < 0.001",
              "LN = late-night. Confidence intervals shown in brackets.",
              "ICC = Intraclass Correlation Coefficient (adjusted)."),
...
modelsummary(
  ...
  add_rows = get_icc_rows(psqi_models_list),
  notes = c(
