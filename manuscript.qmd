...
data.panel <- data.panel |>
  ...
  dplyr::select(pid, wave, date, region, starts_with(c("mctq", "eps", "psqi")),
                ...
                age_scaled, bmi_scaled, SES_index_scaled, msf_sc_numeric, msf_sc_centered, gender) |>
  mutate(
...
# First calculate gaming windows, removing msf_sc_centered to avoid duplication
gamingBiweekly <- calculate_gaming_windows(selfreport |> dplyr::select(-msf_sc_centered), gaming_sessions) |>
  # Then add msf_sc_centered once per participant (not per wave)
  left_join(selfreport |> dplyr::select(pid, msf_sc_centered) |> filter(!is.na(msf_sc_centered)) |> distinct(pid, .keep_all = TRUE),
            by = "pid") |>
...
gamingMonthly <- selfreport |>
  ...
  dplyr::select(-msf_sc_centered) |>
  calculate_gaming_windows(gaming_sessions) |>
  rename(month = wave) |>
  # Add msf_sc_centered once per participant
  left_join(selfreport |> dplyr::select(pid, msf_sc_centered) |> filter(!is.na(msf_sc_centered)) |> distinct(pid, .keep_all = TRUE),
            by = "pid") |>
...
# Get PSQI and WEMWBS from panel data
panel_clean <- read_csv("data/processed/panel_clean.csv.gz", show_col_types = FALSE)

dqc4_person <- panel_clean |>
  filter(wave %in% c(2, 4, 6)) |>
...
outcomes_person_level <- panel_clean |>
...
chronotype_baseline <- panel_clean |>
...
The sample characteristics align well with published norms for adult gamer populations. Self-reported sleep duration of 7.2 hours (SD = 1.1) falls within the typical range of 7.1–7.7 hours reported in systematic reviews of adult gamers [@kemp2021sleep; @derosa2024videogaming]. Mean daytime sleepiness (ESS = 5.6, SD = 3.5) was slightly elevated relative to general population norms (ESS ≈ 4.6, SD ≈ 2.8), with the majority scoring below the clinical cut-off of 10. The mean PSQI global score of 6.7 (SD = 2.8) and 64% prevalence of poor sleepers is consistent with gamer samples (40–60%) and higher than general populations (36%; @hinz2017sleep).
...
p_hourly_playtime <- ggplot(hourly_avg, aes(x = start_hour, y = avg_minutes_per_day,
  ...
  labs(
    ...
    subtitle = "Average daily minutes per hour bin (solid = weekday, striped = weekend)",
...
# Original pre-registered: (1 + ln_monthly_avg_minutes_played | pid)
...
# Had to remove random slope to resolve convergence issues
...
if (is_html) {
  ...
  modelsummary(
    ...
    title = "H1: Effects of Late-Night Gaming on Sleep and Wellbeing"
  )
} else {
...
Model syntax: `psqi_6 ~ ln_monthly_avg_minutes_played * msf_sc_centered + age_scaled + bmi_scaled + SES_index_scaled + region + isWeekend + (1 | pid)`
...
Model syntax: `total_hours_sleep ~ ln_monthly_avg_minutes_played * msf_sc_centered + age_scaled + bmi_scaled + SES_index_scaled + region + isWeekend + (1 + ln_monthly_avg_minutes_played | pid) + (1 | gender)`
...
Model syntax: `wemwbs ~ ln_biweekly_avg_minutes_played * msf_sc_centered + age_scaled + bmi_scaled + SES_index_scaled + region + isWeekend + (1 | pid) + (1 | gender)`
...
Model syntax: `epsTotal ~ ln_monthly_avg_minutes_played * msf_sc_centered + age_scaled + bmi_scaled + SES_index_scaled + region + isWeekend + (1 + ln_monthly_avg_minutes_played | pid) + (1 | gender)`
...
if (is_html) {
  ...
  modelsummary(
    ...
    notes = c("* p < 0.05, ** p < 0.01, *** p < 0.001",
              "LN = late-night. Confidence intervals shown in brackets."),
    title = "H2: Chronotype Moderation of Late-Night Gaming Effects"
  )
} else {
  # Use simpler format for PDF and DOCX - use coef_map to control exact order including random effects
  modelsummary(
