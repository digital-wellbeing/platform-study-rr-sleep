...
# Total sample: use raw panel data for outcomes (pre-filtering)
panel_processed <- process_panel("data/raw/survey_biweekly.csv.gz", data.intake)
...
Full regression coefficients are reported in @tbl-diary-h1h2 in the Appendix; diary subsample characteristics are presented in @tbl-diary-demographics.
...
# Temporarily assign to master_data for helper functions
master_data_saved <- if (exists("master_data")) master_data else NULL

diary_summary_rows <- tibble(Characteristic = character(), Total = character())
...
diary_summary_rows <- bind_rows(diary_summary_rows,
  tibble(Characteristic = "N participants",
         Total = as.character(length(diary_analytical_pids))))

diary_summary_rows <- bind_rows(diary_summary_rows,
  tibble(Characteristic = "N diary entries",
         Total = as.character(nrow(diary_ocat))))
...
diary_gender_counts <- diary_person_analytical |>
  count(gender, name = "n_ana") |> filter(!is.na(gender))

for (g in diary_gender_counts$gender) {
  row <- diary_gender_counts |> filter(gender == g)
  diary_summary_rows <- bind_rows(diary_summary_rows,
    tibble(Characteristic = paste0("    ", g),
           Total = sprintf("%d (%.1f%%)", row$n_ana, 100 * row$n_ana / nrow(diary_person_analytical))))
}
...
msf_med <- median(master_data$chronotype_hours, na.rm = TRUE)
msf_iqr <- IQR(master_data$chronotype_hours, na.rm = TRUE)
diary_summary_rows <- bind_rows(diary_summary_rows,
  tibble(Characteristic = "    MSF~sc~ (HH:MM)",
         Total = sprintf("%s (%s)", hours_to_hhmm(msf_med), hours_to_hhmm(msf_iqr))))
...
# === D. Outcomes ===
diary_summary_rows <- bind_rows(diary_summary_rows,
  tibble(Characteristic = "**D. Sleep Quality Distribution**", Total = ""))

# Sleep quality distribution from diary entries
sq_dist <- diary_ocat |>
  mutate(sleep_quality = as.character(sleep_quality_ord)) |>
  count(sleep_quality, name = "n")
sq_n <- sum(sq_dist$n)
...
for (sq in sq_dist$sleep_quality) {
  ...
  diary_summary_rows <- bind_rows(diary_summary_rows,
    tibble(Characteristic = paste0("    ", sq),
           Total = sprintf("%d (%.1f%%)", row$n, 100 * row$n / sq_n)))
}
...
if (!is.null(diary_model_h1_loaded) && !is.null(diary_model_h2_loaded)) {
  ...
  # Filter to non-threshold coefficients
