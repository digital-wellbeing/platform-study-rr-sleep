...
h1a_ln_minutes <- get_h1_effect(
  ...
  term = "ln_monthly_avg_minutes_played_10"
)
...
h1c_ln_minutes <- get_h1_effect(
  ...
  term = "ln_biweekly_avg_minutes_played_10"
)
...
#| fig-cap: "Predicted probability of poor sleep quality as a function of late-night gaming. The line shows the marginal probability of reporting poor sleep quality (Fairly bad or Very bad) at minute-level resolution from 0 to 90 minutes of average daily late-night gaming. The ribbon represents 95% confidence intervals derived from the gaming coefficient SE. Vertical reference lines show the median, mean, 75th percentile, and 90th percentile of the sample distribution. The marginal density plot (top panel) shows the distribution of late-night gaming in the sample. Predictions are population-average probabilities (marginalised over the participant random intercept) derived from the H1a probit CLMM, with other covariates held at their reference/mean values."
#| fig-width: 9
#| fig-height: 6
...
# Use per-10-min granularity from 0 to 9 (= 0 to 90 minutes)
ln_sequence_10 <- seq(0, 9, by = 0.1)

# Keep x-axis ticks at 30-minute intervals (in original minutes)
ln_bins <- c(0, 30, 60, 90)

# Calculate distribution statistics for reference lines (in original minutes)
...
# Compute MARGINAL exceedance probabilities for the CLMM.
# emmeans mode="exc.prob" gives CONDITIONAL predictions (random effect = 0),
# which are near-zero when sigma_u is large. The correct marginal formula for a
# probit random-intercept model is:
#   P(Y > j | x) = 1 - Phi((alpha_j - eta(x)) / sqrt(1 + sigma_u^2))
# where eta(x) = beta_gaming * x (other scaled covariates at mean = 0, reference levels).

sigma_u   <- sqrt(h1a_model$ST$pid[1, 1])
alpha_j   <- h1a_model$alpha["Fairly good|Fairly bad"]   # P(Fairly bad or Very bad)
b_gaming  <- coef(h1a_model)["ln_monthly_avg_minutes_played_10"]
se_gaming <- sqrt(vcov(h1a_model)[
  "ln_monthly_avg_minutes_played_10",
  "ln_monthly_avg_minutes_played_10"
])
denom <- sqrt(1 + sigma_u^2)

lp    <- b_gaming * ln_sequence_10
lp_lo <- (b_gaming - 1.96 * se_gaming) * ln_sequence_10
lp_hi <- (b_gaming + 1.96 * se_gaming) * ln_sequence_10

plot_data <- tibble(
  ln_monthly_avg_minutes_played = ln_sequence_10 * 10,
  exc.prob  = pnorm((alpha_j - lp)    / denom, lower.tail = FALSE),
  asymp.LCL = pnorm((alpha_j - lp_lo) / denom, lower.tail = FALSE),
  asymp.UCL = pnorm((alpha_j - lp_hi) / denom, lower.tail = FALSE)
)
...
h2a_ln_minutes_msf <- get_h2_interaction(
  ...
  term = "ln_monthly_avg_minutes_played_10:msf_sc_centered"
)
...
h2c_ln_minutes_msf <- get_h2_interaction(
  ...
  term = "ln_biweekly_avg_minutes_played_10:msf_sc_centered"
)
...
# Marginal probability computation for H2a (probit CLMM)
h2a_marginal <- tryCatch({
  m <- readRDS("output/models/imputed_model_h2a.rds")
  b_ln <- m$beta["ln_monthly_avg_minutes_played_10"]
  t2 <- m$alpha["Fairly good|Fairly bad"]
  sd_re <- m$ST$pid[1, 1]
  marginal_sd <- sqrt(1 + sd_re^2)
  pp_per_10min <- dnorm((b_ln * 0 - t2) / marginal_sd) * b_ln / marginal_sd * 100
  list(pp_per_10min = pp_per_10min)
}, error = function(e) NULL)
```
...
#| fig-cap: "Predicted wellbeing (SWEMWBS) as a function of late-night gaming, moderated by chronotype. Lines show model-predicted SWEMWBS scores at three levels of chronotype: −1 SD (morning-oriented), mean, and +1 SD (evening-oriented). Ribbons indicate 95% confidence intervals. Predictions are derived from the H2c linear mixed model, with other covariates held at their reference/mean values and random effects set to zero."
#| fig-width: 8
#| fig-height: 5
...
h2c_model <- readRDS("output/models/imputed_model_h2c.rds")
fe <- fixef(h2c_model)
vc <- vcov(h2c_model)

# Chronotype levels: -1 SD, mean (0), +1 SD
chrono_sd <- sd(h2c_model@frame$msf_sc_centered)
