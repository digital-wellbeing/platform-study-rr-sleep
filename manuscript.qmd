...
We summarised the extent of missingness for the key self-report outcomes across each survey wave in the raw data before imputation.
...
wave_missing_counts <- selfreport |>
  ...
  group_by(wave) |>
  summarise(
    Observations = dplyr::n_distinct(pid),
...
modelsummary(
  list(
    "PSQI Global (Imputed)" = model_psqi_global_imputed,
    "PSQI Global (Complete-Case)" = model_psqi_global_completecase
  ),
  fmt = 2,
  estimate  = "{estimate} [{conf.low}, {conf.high}]{stars}",
  statistic = NULL,
